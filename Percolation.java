...
public class Percolation {
	...
	static void init() {
		for (int i = 0; i < length - 2; i++)
			grid[i] = false; // false pour BLANC et true pour NOIR
		grid[length - 2] = grid[length - 1] = true;
		UnionFind.init(length);
	}

	static void propagateUnion(int x) {
		if ((x + 1 >= 0) && (x + 1 < length) && ((x + 1) / size == x / size) && grid[x + 1])
	...
	static boolean detectPath(boolean[] seen, int n, boolean up) {
		...
		if ((n - size >= 0) && (n - size < length)) {
			if (detectPath(seen, n - size, up))
				return true;
		}
		if ((n - 1 >= 0) && (n - 1 < length) && ((n - 1) / size == n / size)) {
